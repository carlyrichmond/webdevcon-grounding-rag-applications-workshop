...
RAG, or Retrieval Augmented Generation, is a software architecture where documents retrieved from a database or search engine are use as the context to an LLM to help it generate a more accurate and relevant response. It is a common architectural pattern used to ground an LLM.

## What is Grounding?

Grounding is the process of providing additional data that the LLM has not been trained on to ensure responses contain correct and domain-specific information, commonly using vector search (but not always):

![RAG Architecture | Source: Elastic](./screenshots/6/6-elastic-retrieval-augmented-generation.png)
